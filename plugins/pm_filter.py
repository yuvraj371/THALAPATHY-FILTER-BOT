...
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_shortlink, get_tutorial, send_all, get_cap, convert_time
...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
       ...
       f"\n\nनमस्कार {message.from_user.mention}, आपकी request 🍿 {search} के लिए मेरे डेटाबेस में {str(total_results)} परिणाम मिले हैं।\n\n"
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if FIRST_CHANNEL and SECOND_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Jᴏɪɴ ᴏᴜʀ Updates and Movie ᴄʜᴀɴɴᴇʟ! 😒", show_alert=True)
            return
        ...
        btn2 = [[
                 InlineKeyboardButton('❤️Jᴏɪɴ Cʜᴀɴɴᴇʟ❤️', url=link.invite_link),
                 InlineKeyboardButton("Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")
               ],[
                 InlineKeyboardButton("🎥Movie group", url="https://t.me/+Qn6fthcb7wI0ZTk1")
               ]]
        ...
        try:
            ...
            await log_msg.reply_text(
                text=f"#ʟɪɴᴋ_ɢᴇɴᴇʀᴀᴛᴇᴅ ꜰᴏʀ ɪᴅ - {user_id} \n•• ᴜꜱᴇʀɴᴀᴍᴇ : {username} \n\n•• ᖴᎥᒪᗴ Nᗩᗰᗴ : {fileName}",
                quote=True,
                disable_web_page_preview=True,
    ...
    # don't change anything without contacting me @creatorrio
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
                    InlineKeyboardButton("😇Cᴏɴᴛᴀᴄᴛ😇", url="telegram.me/Madhuri_niranjan")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
